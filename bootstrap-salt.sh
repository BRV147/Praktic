...
echo ".*${PROJECT}[-.a-z0-9]{1,}-[0-9]{19}" > /etc/salt/autosign.conf
cp /etc/salt/master /etc/salt/master.bkp
sed -i "s/#interface: 0.0.0.0/${REPLACE}/g" /etc/salt/master
sed -i 's/#autosign_file/autosign_file/g' /etc/salt/master
sed -i "s/${LOGFILE}/${NEWLOG}/g" /etc/salt/master
sed -i 's/#key_logfile:/key_logfile:/g' /etc/salt/master
sed -i 's/#log_level: warning/log_level: debug/g' /etc/salt/master
...
gitfs_remotes:
  - https://github.com/BRV147/saltstate.git
...
base:
  '*':
    - powerdns
    - powerdns.conf_master" >> /srv/salt/top.sls

echo "base:
  '*':
    - powerdns" >> /srv/pillar/top.sls
exit 0
